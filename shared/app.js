// ============================================================
// shared/app.js — チームみらい 活動戦略ダッシュボード ロジック
...
// These are populated by initApp() after CSV data is loaded
const AREA_RATE_MAP = {};
let AREA_NAMES = [];

function buildAreaMaps() {
  // Clear and rebuild
  Object.keys(AREA_RATE_MAP).forEach(k => delete AREA_RATE_MAP[k]);
...
window.addEventListener('load', () => {
  ...
  if (saved) document.getElementById('sheets-url-input').value = saved;
  addSpeechEntry();
});
...
function saveSheetUrl() {
  ...
  if (!url.startsWith('https://script.google.com')) {
    alert('Google Apps Script のデプロイURLを入力してください\n（https://script.google.com/macros/s/... の形式）');
    return;
  }
  localStorage.setItem('speechSheetUrl', url);
  loadSpeechData();
}

async function loadSpeechData() {
  const url = localStorage.getItem('speechSheetUrl') ||
              document.getElementById('sheets-url-input').value.trim();
  if (!url) { setStatus('gray','URLを入力してください',''); return; }
  setStatus('yellow','読み込み中...',url.slice(0,48)+'...');
  document.getElementById('speech-reload-btn').textContent='読込中';
  try {
    ...
    setStatus('red','読み込み失敗: '+e.message,'URLとApps Scriptの設定を確認してください');
  }
  document.getElementById('speech-reload-btn').textContent='再読込';
}

function setStatus(color, text, sub) {
  const colors={green:'#4ade80',yellow:'#fbbf24',red:'#f87171',gray:'#6b7280'};
  document.getElementById('speech-status-dot').style.background = colors[color]||colors.gray;
...
function useSampleData() {
  const sample = [
    {date:'2025/10/01',place:'武蔵小杉駅前',area:'川崎市中原区',vip:true, type:'街頭演説',pax:80},
    {date:'2025/10/08',place:'武蔵小杉駅前',area:'川崎市中原区',vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/10/15',place:'武蔵小杉駅前',area:'川崎市中原区',vip:true, type:'街頭演説',pax:120},
    {date:'2025/10/23',place:'武蔵小杉駅前',area:'川崎市中原区',vip:false,type:'街宣車遊説',  pax:null},
    {date:'2025/10/03',place:'新横浜駅前',  area:'横浜市港北区',vip:true, type:'街頭演説',pax:60},
    {date:'2025/10/17',place:'新横浜駅前',  area:'横浜市港北区',vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/10/31',place:'日吉駅前',    area:'横浜市港北区',vip:false,type:'ももたろう',pax:null},
    {date:'2025/10/05',place:'たまプラーザ駅前',area:'横浜市青葉区',vip:true, type:'街頭演説',pax:90},
    {date:'2025/10/19',place:'たまプラーザ駅前',area:'横浜市青葉区',vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/10/10',place:'辻堂駅前',    area:'藤沢市',      vip:true, type:'街頭演説',pax:70},
    {date:'2025/10/24',place:'辻堂駅前',    area:'藤沢市',      vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/10/12',place:'茅ヶ崎駅北口',area:'茅ヶ崎市',   vip:true, type:'街頭演説',pax:50},
    {date:'2025/10/26',place:'茅ヶ崎駅北口',area:'茅ヶ崎市',   vip:false,type:'街宣車遊説',  pax:null},
    {date:'2025/10/07',place:'溝の口駅前',  area:'川崎市高津区',vip:true, type:'街頭演説',pax:65},
    {date:'2025/10/21',place:'溝の口駅前',  area:'川崎市高津区',vip:false,type:'ももたろう',pax:null},
    {date:'2025/10/14',place:'センター北駅前',area:'横浜市都筑区',vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/10/09',place:'登戸駅前',    area:'川崎市多摩区',vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/11/02',place:'○○ホール',  area:'横須賀市',    vip:true, type:'個人演説会',pax:150},
    {date:'2025/11/05',place:'平塚駅前',    area:'平塚市',      vip:false,type:'街頭演説',pax:null},
    {date:'2025/11/08',place:'大和駅前',    area:'大和市',      vip:false,type:'街頭活動（チラシ配り等）',pax:null},
    {date:'2025/11/09',place:'大和駅前',    area:'大和市',      vip:true, type:'個人演説会',pax:80},
  ];
  processSpeechData(sample);
  setStatus('green',`サンプル ${sample.length}件`,'実際のデータはURLを設定してください');
}
...
function addSpeechEntry(){
  const id=Date.now()+Math.random(); _manualEntries.push({id}); renderManualEntries();
}
function removeManualEntry(id){
  _manualEntries=_manualEntries.filter(e=>e.id!==id); renderManualEntries();
}
function renderManualEntries(){
  document.getElementById('speech-entries').innerHTML=_manualEntries.map(e=>`
    <div data-id="${e.id}" style="margin-bottom:10px;padding:9px;background:#f9fafb;border-radius:9px;border:1px solid var(--border);">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;margin-bottom:5px;">
        <input class="sp-place" type="text" placeholder="場所名（例：武蔵小杉駅前）"
          style="padding:7px 8px;border:1px solid var(--border);border-radius:8px;font-size:11px;font-family:'Noto Sans JP',sans-serif;background:var(--white);color:var(--text);">
        <input class="sp-area" type="text" placeholder="自治体名" list="dl-${e.id}"
          style="padding:7px 8px;border:1px solid var(--border);border-radius:8px;font-size:11px;font-family:'Noto Sans JP',sans-serif;background:var(--white);color:var(--text);">
        <datalist id="dl-${e.id}">${AREA_NAMES.map(a=>`<option value="${a}">`).join('')}</datalist>
      </div>
      <div style="display:grid;grid-template-columns:1fr 60px 70px 26px;gap:5px;align-items:center;">
        <select class="sp-type"
          style="padding:7px 8px;border:1px solid var(--border);border-radius:8px;font-size:11px;font-family:'Noto Sans JP',sans-serif;background:var(--white);color:var(--text);">
          ${SPEECH_TYPES.map(t=>`<option value="${t}">${t}</option>`).join('')}
        </select>
        <input class="sp-count" type="number" placeholder="回数" min="1"
          style="padding:7px 5px;border:1px solid var(--border);border-radius:8px;font-size:11px;text-align:center;font-family:'DM Mono',monospace;background:var(--white);color:var(--text);">
        <input class="sp-pax" type="number" placeholder="人数(任意)" min="0"
          style="padding:7px 5px;border:1px solid var(--border);border-radius:8px;font-size:11px;text-align:center;font-family:'DM Mono',monospace;background:var(--white);color:var(--text);">
        <button onclick="removeManualEntry(${e.id})"
          style="width:26px;height:26px;border-radius:50%;border:1px solid #fca5a5;background:#fee2e2;color:#dc2626;font-size:14px;cursor:pointer;flex-shrink:0;">×</button>
      </div>
    </div>`).join('');
}
function analyzeManualSpeech(){
  const rows=document.querySelectorAll('#speech-entries [data-id]');
  const rawData=[];
  rows.forEach(row=>{
    const place = row.querySelector('.sp-place').value.trim();
    const area  = row.querySelector('.sp-area').value.trim();
    const type  = row.querySelector('.sp-type').value || 'その他';
    const count = parseInt(row.querySelector('.sp-count').value)||0;
    const paxVal= parseInt(row.querySelector('.sp-pax').value);
    const pax   = isNaN(paxVal) ? null : paxVal;
    if(!area||count<=0) return;
    for(let i=0;i<count;i++) rawData.push({place,area,type,vip:false,pax:i===0?pax:null,date:''});
  });
  if(rawData.length===0){alert('自治体名と回数を入力してください');return;}
  processSpeechData(rawData);
  setStatus('green',`手動入力 ${rawData.length}件`,'');
}
