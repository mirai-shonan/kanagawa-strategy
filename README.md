...
ブラウザで http://localhost:8000/ を開きます（自動的に神奈川版へリダイレクト）。
直接アクセスする場合は http://localhost:8000/prefectures/kanagawa/ も可能です。
...
1. **`prefectures/kanagawa/index.html` を編集**
   ...
   - **タブのIDは英語**（`overview`, `all`, `compare`, `elected`, `cost`, `growth`, `events`, `posting`, `speech`, `academic`, `family`, `senior`）。新しい県でも同じID体系を維持してください
